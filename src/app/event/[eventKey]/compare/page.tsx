...
export default async function ComparePage({ params }: { params: Promise<{ eventKey: string }> }) {
    ...
    const teams = Array.from(new Set(reports.map(r => r.teamKey))).map(teamKey => {
        ...
        const avgTowerPts = teamReports.reduce((acc, r) => acc + (TELE_TOWER[r.data.teleop.tower_level] || 0), 0) / teamReports.length;
        const towerRate = teamReports.filter(r => r.data.teleop.tower_level !== 'None').length / teamReports.length * 100;
